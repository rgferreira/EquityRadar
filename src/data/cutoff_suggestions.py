...
def suggestion_signature(tickers: list[str]) -> str:
    payload = f"v2|{date.today().isoformat()}|{'|'.join(sorted(set(tickers)))}"
...
def _price_candidates(ticker: str, history: pd.DataFrame, market: bool = False) -> list[dict[str, object]]:
    ...
    for timestamp in returns.abs().nlargest(2).index:
        ...
        candidates.append({
            ...
            "trigger_type": "Market regime" if market else "Watchlist movement",
...
def generate_cutoff_suggestions(tickers: list[str], db_path: str | Path | None = None) -> list[dict[str, object]]:
    ...
    for ticker in tickers:
        try:
            candidates.extend(_price_candidates(ticker, fetch_price_history(ticker, period="3y")))
        except Exception:
            continue
        ...
        if eligible:
            ...
            candidates.append({
                ...
                "trigger_type": "Watchlist positioning",
    ...
    for candidate in (item for item in ranked if item["trigger_type"] == "Market regime"):
        add_novel(candidate)
        if sum(item["trigger_type"] == "Market regime" for item in chosen) == 2:
            break
...
def schedule_cutoff_suggestions(tickers: list[str], db_path: str | Path | None = None) -> bool:
    ...
    with _lock:
        if fresh or (_future and not _future.done()):
            return False
