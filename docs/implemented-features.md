...
- Phase 3.9 operational acceptance: prospective post-promotion monitoring; all-page AppTest plus real desktop/phone Chromium regression; per-provider freshness, cooldown, in-flight, isolated-error and recovery diagnostics; server-lifetime scheduled refresh; verified daily SQLite backups with isolated restore drills; diagnostic-boundary, FINRA-reversal, provider-recovery and matured-lesson alerts; options-continuity gates; and a non-executing allocation Scenario lab.
